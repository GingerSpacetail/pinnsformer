...
class SEIRPINNSolver:
    ...
    def __init__(self, model_params, device='cpu', network_type='ode_pinn'):
        ...
        if network_type == 'pinn':
            self.net = PINNs(in_dim=1, hidden_dim=128, out_dim=4, num_layer=4).to(device)
            self.net.apply(self._init_weights)
        elif network_type == 'ode_pinn':
            self.net = create_ode_pinn('seir', out_dim=4, hidden_dim=128, num_layer=4, enhanced=True).to(device)
            init_ode_pinn_weights(self.net)
        elif network_type == 'pinnsformer':
            self.net = PINNsformer(d_out=4, d_hidden=256, d_model=32, N=2, heads=4).to(device)
            self.net.apply(self._init_weights)
        else:
            raise ValueError("network_type must be 'pinn', 'ode_pinn', or 'pinnsformer'")
        
        # Optimizer
        self.optimizer = LBFGS(self.net.parameters(), line_search_fn='strong_wolfe')
        
        print(f"Network parameters: {get_n_params(self.net)}")
